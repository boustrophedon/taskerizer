...
use crate::task::test_utils::{example_task_1, example_task_break_1, arb_task};
...
proptest! {
    ...
    fn test_db_add_arb(task1 in arb_task(),
        ...
        prop_assert!(tx.add_task(&task1).is_ok(), "Adding task failed. task1: {:?}", task1);
        prop_assert!(tx.add_task(&task2).is_ok(), "Adding task failed. task2: {:?}", task2);
    }
}
